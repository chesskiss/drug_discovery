...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Directory holding the per-run CV output dirs (pca{K}_{split}, nogenes_{split}).",
    )
    parser.add_argument(
        "--output",
        type=str,
...
def collect_points(sweep_dir: Path, metric: str = "rmse") -> dict[str, dict[str, list]]:
    ...
    for run_dir in sorted(sweep_dir.iterdir()):
        ...
        if rmse is None:
            continue
        pca_match = re.match(r"pca(\d+)_(.+)", name)
        fs_match = re.match(r"featsel(\d+)_(.+)", name)
        nog_match = re.match(r"nogenes_(.+)", name)
        if pca_match:
            dim, split = int(pca_match.group(1)), pca_match.group(2)
            if split in points:
...
def print_table(points: dict[str, dict[str, list]], metric: str = "rmse") -> None:
    ...
    for split in SPLIT_STYLE:
        ...
        for dim, rmse, err in zip(p["dims"], p["rmse"], p["err"]):
            print(f"{split:<26}{dim:>6}{rmse:>12.4f}{err:>10.4f}   [pca]")
        if p["nogenes"] is not None:
            m, e = p["nogenes"]
