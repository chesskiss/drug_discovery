...
def build_cv_dataset_bundle(
    ...
    *,
    ...
    if split_strategy == "random":
        ...
        # Group-aware folds over cell lines only (drugs are not excluded). Reuses the
        # priority router with empty drug sets so held-out cell lines never leak.
        cell_folds = split_unique_values_into_folds(
            cv_df["cell_line"].astype(str).tolist(),
            num_folds=num_folds,
            random_seed=seed,
        ...
        if train_rows.empty or val_rows.empty or test_rows.empty:
            raise ValueError(
                "cell_line CV produced an empty split "
                f"for seed={seed}, fold={fold_idx + 1} "
                f"(train={len(train_rows)}, val={len(val_rows)}, test={len(test_rows)})."
            )
        train_rows = train_rows.reset_index(drop=True)
