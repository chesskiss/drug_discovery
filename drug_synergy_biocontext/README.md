# Drug Synergy — Bio-Context Model

Fork of `drug_synergy_baseline/` in which **PCA compression is replaced by a fixed,
biologically-derived pathway projection**.

## What changed vs the baseline

The cell-line branch is now:

```
raw CellLine[0] (23808)  --W-->  pathway activities  --z-score-->  MLP0  -->  concat with drugs
```

- `W` is a **fixed** `[n_pathways x 23808]` matrix from `data/bio_context/`
  (PROGENy = 14, KEGG = 336 after dropping 36 all-zero rows, `progeny_kegg` = 350).
  It is not fitted to this dataset, and its rows are **named pathways**.
- **MLP0** is the trainable compression head; its output width is the swept parameter
  (`--mlp0-out-dim`), playing the role PCA dimension played in the baseline.
  `--mlp0-out-dim 0` disables the cell-line branch entirely (no-genes control).
- Pathway activities are **z-scored using TRAIN cell lines only**. Raw activities are
  ~1e4x larger than the L2-normalised drug features, so this is required, not cosmetic.

Removed relative to the baseline: `--cell-expression-path`, `--gene-feature-set`,
`--cell-feature-view`, the pre-built PCA CSVs, and the `run_pca_sweep*` / `run_featsel*`
scripts. Expression is always read from the raw pickle view 0 and projected through `W`.

## Commands

```bash
# smoke test (minutes) -> writes outputs/training_curves/smoke_progeny.png
bash scripts/smoke_progeny.sh

# PROGENy sweep, 10-fold CV, 3 splits (dims capped at 14 = number of pathways)
bash scripts/run_progeny_sweep.sh

# plot any sweep directory
uv run --directory . python -m src.biocontext_rmse_plot \
  --sweep-dir outputs/progeny --eval-name "10-fold CV" --metric rmse \
  --output outputs/training_curves/progeny_rmse.png
```

Run dirs are named `mlp0{K}_{split}` so the plot module can parse the swept width.
